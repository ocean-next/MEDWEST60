MODULE stopar
   ...
   LOGICAL, PUBLIC :: ln_sto_e3  = .FALSE.    ! stochastic vertical metric
   INTEGER, PUBLIC :: jsto_e3                 ! index of stochastic vertical metric
   REAL(wp)        :: rn_e3_std               ! standard deviation (in percent)
   REAL(wp)        :: rn_e3_tcor              ! correlation timescale (in timesteps)
   INTEGER         :: nn_e3_flt = 0           ! number of passes of Laplacian filter
   INTEGER         :: nn_e3_ord = 1           ! order of autoregressive processes
   ...
   REAL(wp)        :: rn_hgr_std1             ! standard deviation (in percent)
   REAL(wp)        :: rn_hgr_std2             ! standard deviation (in percent)
   ...
   !! $Id: stopar.F90 2528 2010-12-27 17:33:53Z rblod $
...
CONTAINS
   ...
   SUBROUTINE sto_par( kt )
      ...
      IF (nmember==1) THEN
        IF (narea==82) THEN
              PRINT*,"== SLX stopat debut"
              IF (ln_sto_e3)  PRINT*, sto3d(:,1,1,1)
              IF (ln_sto_eos)  PRINT*, sto2d(:,1,1)
              PRINT*,"== SLX stopar debut. DOne"
        END IF
      END IF
      ...
      IF (ln_sto_eos) THEN
        IF (nmember==1) THEN
        IF (narea==82) THEN
              PRINT*,"== SLX stopar mid"
              PRINT*, sto2d(:,1,1)
              PRINT*,"== SLX stopar mid. DOne"
        END IF
      END IF
      END IF
      ...
      IF (nmember==1) THEN
        IF (narea==82) THEN
              PRINT*,"== SLX stopar fin"
               IF (ln_sto_e3) PRINT*, sto3d(:,1,1,1)
               IF (ln_sto_eos) PRINT*, sto2d(:,1,1)
              PRINT*,"== SLX stopar fin. DOne"
        END IF
      END IF
   ...
   SUBROUTINE sto_par_init
      ...
      NAMELIST/namsto/ ln_sto_ldf, rn_ldf_std, rn_ldf_tcor, &
        &              ln_sto_e3, rn_e3_std, rn_e3_tcor, nn_e3_flt, nn_e3_ord, &
        &              ln_sto_hgr, rn_hgr_std1, rn_hgr_std2, rn_hgr_tcor, &
        &              nn_hgr_flt, nn_hgr_ord, &
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsto in configuration namelist', lwp )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      stochastic vertical metric              ln_sto_e3    = ', ln_sto_e3
         WRITE(numout,*) '      vertical metric std (in percent)        rn_e3_std    = ', rn_e3_std
         WRITE(numout,*) '      vertical metric tcor (in timesteps)     rn_e3_tcor   = ', rn_e3_tcor
         WRITE(numout,*) '      order of autoregressive  processes      nn_e3_ord    = ', nn_e3_ord
         WRITE(numout,*) '      passes of Laplacian filter              nn_e3_flt    = ', nn_e3_flt

         WRITE(numout,*) '      stochastic horizontal grid              ln_sto_hgr  = ', ln_sto_hgr
         WRITE(numout,*) '      std1 (in percent)                        rn_hgr_std1  = ', rn_hgr_std1
         WRITE(numout,*) '      std2 (in percent)                        rn_hgr_std2  = ', rn_hgr_std2
      ...
      jpsto3d = 0
      IF( ln_sto_e3 ) THEN
         IF(lwp) WRITE(numout,*) '       - stochastic vertical metric'
         jpsto3d   = jpsto3d + nn_e3_ord
         jsto_e3   = jpsto3d
      ENDIF
      IF( ln_sto_trd ) THEN
      ...
      DO jsto = 1, jpsto3d
         ...
         sto3d_flt(jsto) = 0
         sto3d_ord(jsto) = 1
         DO jord = 0, nn_e3_ord-1
            IF ( jsto+jord == jsto_e3 ) THEN ! Stochastic vertical metric
               sto3d_ord(jsto) = nn_e3_ord - jord
               sto3d_flt(jsto) = nn_e3_flt
            ENDIF
         ENDDO
      ...
      DO jsto = 1, jpsto2d
         ...
         ENDIF
         DO jord = 0, nn_hgr_ord-1
            IF ( jsto+jord == jsto_hgr1 ) THEN ! Stochastic horizontal grid (ave=1)
               sto2d_ave(jsto)  = 0._wp
               sto2d_std(jsto) = rn_hgr_std1
               sto2d_tcor(jsto) = rn_hgr_tcor
            ENDIF
            IF ( jsto+jord == jsto_hgr2 ) THEN ! Stochastic horizontal grid (ave=1)
               sto2d_ave(jsto)  = 0._wp
               sto2d_std(jsto) = rn_hgr_std2
               sto2d_tcor(jsto) = rn_hgr_tcor
            ENDIF
         ENDDO
         DO jdof = 1, nn_sto_eos
      ...
      DO jsto = 1, jpsto3d
         ...
         DO jord = 0, nn_e3_ord-1
            IF ( jsto+jord == jsto_e3 ) THEN ! Stochastic vertical metric (ave=1)
               sto3d_ave(jsto) = 0._wp
               sto3d_std(jsto) = rn_e3_std
               sto3d_tcor(jsto) = rn_e3_tcor
            ENDIF
         ENDDO
      ...
      DO jsto = 1, jpsto2d
         ...
         !SLX: sto2d(:,:,jsto) = SIGN(MIN(sto2d_lim(jsto),ABS(sto2d(:,:,jsto))),sto2d(:,:,jsto))
      ...
      IF (nmember==1) THEN
        IF (narea==82) THEN
              PRINT*,"== SLX stoparinit"
              IF (ln_sto_e3)  PRINT*, sto3d(:,1,1,1)
              IF (ln_sto_eos) PRINT*, sto2d(:,1,1)
              PRINT*,"== SLX stoparinit. DOne"
        END IF
      END IF
   ...
   SUBROUTINE sto_rst_write( kt )
      ...
      IF ( jpsto2d > 0 .OR. jpsto3d > 0 ) THEN
         ...
         IF( kt == nitrst .OR. kt == nitend ) THEN
            ...
            ENDIF
         ENDIF
      !PRINT*,'sto_rst_write : write stochastic parameters in restart file'
         ...
         IF( kt == nitrst - 1 .OR. nstock == 1 .OR. kt == nitend-1 ) THEN
            ...
            ENDIF
            ! open the restart file
      CALL iom_open(TRIM(cn_ocerst_outdir)//"/"//clname, numstow, ldwrt = .TRUE.,kiolib=jprstlib)
         ENDIF
         ENDIF
