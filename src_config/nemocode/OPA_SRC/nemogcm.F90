...
904   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namcfg in configuration namelist', .TRUE. )   
      ...
      READ  ( numnam_ref, nammpp, IOSTAT = ios, ERR = 905)
905   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nammpp in reference namelist', .TRUE. )
      ...
      READ  ( numnam_cfg, nammpp, IOSTAT = ios, ERR = 906 )
906   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nammpp in configuration namelist', .TRUE. )
...
! Force values for AGRIF zoom (cf. agrif_user.F90)
   ...
   ENDIF
      ...
      IF( ln_ensemble ) THEN
        narea = mynode( cltxt, 'output.namelist.dyn', numnam_ref, numnam_cfg, numond , nstop, ilocal_comm, kmember=nmember )
        WRITE(cn_mem,'(I3.3)') nmember
        cxios_context = TRIM(cn_mem)//TRIM(cxios_context)
      ELSE
      ...
      ELSE
         ...
         ! Nodes selection (control print return in cltxt)
         IF( ln_ensemble ) THEN
           narea = mynode( cltxt, 'output.namelist.dyn', numnam_ref, numnam_cfg, numond , nstop, kmember=nmember )
           WRITE(cn_mem,'(I3.3)') nmember
         ELSE
      ...
      IF(lwp) THEN                            ! open listing units
         ...
         IF(ln_ensemble) cloutput = cn_mem//TRIM(cloutput)
