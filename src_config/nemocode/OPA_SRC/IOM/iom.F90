...
# endif
   ...
   !!INTEGER, PUBLIC,                   PARAMETER :: nk_us = 10                !lolo: how many levels for "user-selected" depth !
   !!                                                          25  50  75 100 150 200 300 400 500 600    m
   !!INTEGER, PUBLIC, DIMENSION(nk_us), PARAMETER :: jk_us = (/ 13, 20, 26, 30, 38, 45, 56, 65, 73, 81 /)   !LOLO: eNATL60 300level / k index corresponding to nearest point to the depth... (100m, 500m)
   !!
   !INTEGER, PUBLIC,                   PARAMETER :: nk_us = 5                !lolo: how many levels for "user-selected" depth !
   !                                                          25  50 100 200 400 m
   !INTEGER, PUBLIC, DIMENSION(nk_us), PARAMETER :: jk_us = (/ 13, 20, 30, 45, 65 /)   !LOLO: eNATL60 300level / k index corresponding to nearest point to the depth... (100m, 500m)
...
CONTAINS
   ...
   SUBROUTINE iom_init( cdname ) 
      ...
      !!REAL(wp), ALLOCATABLE, DIMENSION(:)   :: vdepth_us !lolo: vector depth corresponding to index "jk_us"
      !!----------------------------------------------------------------------
#if ! defined key_xios2
...
# endif
      ...
      !LOLO:
      !ALLOCATE ( vdepth_us(nk_us) )
      !vdepth_us(ji) = 0.0_wp
      !DO ji=1, nk_us
      !   vdepth_us(ji) = gdept_1d(jk_us(ji))
      !END DO
      !IF ( lwp ) THEN
      !   WRITE(numout,*) ' LOLO: iom.F90 : jk_us(:) =', jk_us(:)
      !   WRITE(numout,*) ' LOLO: iom.F90 : vdepth_us(:) =', vdepth_us(:)
      !END IF
      !CALL iom_set_axis_attr( "deptht_us", vdepth_us ) ; ! lolo
      !IF ( lwp ) WRITE(numout,*) ' LOLO: iom.F90 : vdepth_us(:) was just fed to "iom_set_axis_attr" as "deptht_us" !'
      !DEALLOCATE ( vdepth_us )
      !LOLO.
