MODULE iom_nf90
   ...
   END INTERFACE iom_nf90_getatt
   INTERFACE iom_nf90_rstput
      MODULE PROCEDURE iom_nf90_rp0123d
   END INTERFACE iom_nf90_rstput
...
CONTAINS
   ...
   SUBROUTINE iom_nf90_open( cdname, kiomid, ldwrt, ldok, kdompar )
      ...
      ELSE                                       ! the file does not exist (or we overwrite it)
         ...
         IF( ldwrt ) THEN  ! the file should be open in write mode so we create it...
            IF( jpnij > 1 ) THEN
               !LOLO: !WRITE(cltmp,'(a,a,i4.4,a)') cdname(1:iln-1), '_', narea-1, '.nc'
   ...
   SUBROUTINE iom_nf90_rp0123d( kt, kwrite, kiomid, cdvar , kvid  , ktype,   &
      ...
      !  Additional local variables for the token circulation (JMM)
      INTEGER, DIMENSION(MPI_STATUS_SIZE) :: istatus
      INTEGER               :: itoken, itag=100, icolor, ilen, ipos
      INTEGER               :: impierr, ilocalcomm
      INTEGER               :: impirank_loc, impisize_loc
      INTEGER               :: inext, iprev
      ...
      IF( kt == kwrite ) THEN
         ...
         ! =============
         !JMM TEST
         !  Split the mpi_comm_opa communicator into 'node' communicator
         !   ( group cores belonging to one compute node in a dedicated communicator (localcomm) )
         ! (1) find node name fo the current rank
         CALL MPI_GET_PROCESSOR_NAME(clname, ilen, impierr)
         ! (1.1) Transform the node name into a 'color' (integer).
         ! erase extension in the name
         ipos=INDEX( clname,'.' )
         IF ( ipos /= 0 ) THEN
            clname=TRIM(clname(1:ipos-1))
         ENDIF
         icolor=name_to_color(clname)
         !        print *, ' >>> NAREA : icolor clname', narea, icolor, TRIM(clname)
         ! (2) Split the mpi_comm_opa according to icolor. Key is 0
         CALL MPI_COMM_SPLIT(mpi_comm_opa,icolor,0,ilocalcomm, impierr)
         !        print *, ' >>> NAREA : ilocalcomm, impierr', narea, ilocalcomm, impierr
         ! (3) Now set the local rank and local size
         CALL MPI_COMM_RANK(ilocalcomm, impirank_loc, impierr)
         CALL MPI_COMM_SIZE(ilocalcomm, impisize_loc, impierr)
         !        print *, ' >>> NAREA : impirank_loc impisize_loc', narea, impirank_loc,impisize_loc
         ! (4) barrier in localcomm just in case
         CALL MPI_BARRIER(ilocalcomm, impierr)
         ! == End of CommSplit ==
         inext=impirank_loc+1
         iprev=impirank_loc-1
         IF ( inext > impisize_loc -1 ) inext=0               ! if last core of the comm send to 0
         IF ( iprev < 0               ) iprev=MPI_PROC_NULL   ! send/recv does nothing !


         ! receive token from mpirank -1
         CALL MPI_RECV(itoken,1,MPI_INTEGER,iprev,itag,ilocalcomm,istatus,impierr)
         !   do some work
         ...
         ENDIF
         !  send token to mpirank +1
         !        print *,'>>> ', narea -1 ,impirank_loc, 'SEND to ', inext
         CALL MPI_SEND(itoken,1,MPI_INTEGER,inext,itag,ilocalcomm,impierr)

         CALL MPI_BARRIER(mpi_comm_opa,impierr)
         CALL MPI_COMM_FREE(ilocalcomm,impierr)
   ...
   !INTEGER(kind=4)  FUNCTION name_to_color(cdname)
   !   CHARACTER(*), INTENT(in) :: cdname
   !   CHARACTER(LEN=1) :: cl1
   !   INTEGER :: ji
   !   name_to_color=0
   !   DO ji= LEN(TRIM(cdname)),1,-1
   !      cl1=TRIM(cdname(ji:ji+1))
   !      name_to_color=75*name_to_color +iachar(cl1)-47
   !   ENDDO
   !END FUNCTION name_to_color


   INTEGER(kind=4)  FUNCTION name_to_color(cdname)
   ! SLX : 18/06/2019 : adapted this function to the format of 
   ! processor names as given on ADA/cines
   ! SLX : 03/10/2019 : adapted this function to the format of 
   ! processor names as given on JZAY/cines
      ...
      INTEGER :: is, ir, ib
      name_to_color=0
   !   READ(cdname,"(1x,i2.2,1x,i1.1,1x,i2.2)") is, ir, ib
   !   READ(cdname,"(1x,'ada',i3)") ib
   PRINT*,'SLX print ',cdname
   !READ(cdname,"(1x,'ada',i3)") ib
   !name_to_color=ib+100*ir+10000*is
   name_to_color=ib
   END FUNCTION name_to_color
