MODULE domvvl
   ...
   USE timing          ! Timing
   USE stopar          ! Stochastic parameters
...
CONTAINS
   ...
   SUBROUTINE dom_vvl_sf_nxt( kt, kcall ) 
      ...
      ! PRINT*,"== SLX vvl"
      ! PRINT*,NAREA                               !
      ! PRINT*,' MAXVAL(sto3d) =', MAXVAL( sto3d(:,:,:,1) )
      ! PRINT*,' MAXVAL(fse3t_a) =', MAXVAL( fse3t_a(:,:,:) )
      ! PRINT*,' MINVAL(sto3d) =',MINVAL(sto3d(:,:,:,1))
      ! PRINT*,' MINVAL(fse3_a) =',MINVAL( fse3t_a(:,:,:))
      ! PRINT*, sto3d(:,1,1,1)
      ! END IF
      ! END IF
    
      !IF (nmember==1) THEN 
      !IF (MINVAL(sto3d(:,:,:,1))==0.) PRINT*,'minval(sto3d is zero) for NAREA= ',NAREA
      !END IF
      ...
      IF (ln_sto_e3) THEN
        fse3t_a(:,:,:) = fse3t_a(:,:,:) *(1._wp + sto3d(:,:,:,jsto_e3))
      END IF
