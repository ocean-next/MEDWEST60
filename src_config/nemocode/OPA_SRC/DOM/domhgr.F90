...
! some point are undefined but you need to have e1 and e2 .NE. 0
    ...
    SUBROUTINE sto_hgr
      ...
      IF (ln_sto_hgr) THEN
         ...
         CALL lbc_lnk( e1t(:,:), 'T' , 1._wp )
         CALL lbc_lnk( e2t(:,:), 'T' , 1._wp )
         CALL lbc_lnk( e1u(:,:), 'U' , 1._wp )
         CALL lbc_lnk( e2u(:,:), 'U' , 1._wp )
         CALL lbc_lnk( e1v(:,:), 'V' , 1._wp )
         CALL lbc_lnk( e2v(:,:), 'V' , 1._wp )
         CALL lbc_lnk( e1f(:,:), 'F' , 1._wp )
         CALL lbc_lnk( e2f(:,:), 'F' , 1._wp )
         ...
         ! Useful shortcuts (JC: note the duplicated e2e2t array ! Need some
         ! cleaning)
         ! ---------------------------------------------------------------------------
         e12t    (:,:) = e1t(:,:) * e2t(:,:)
         e12u    (:,:) = e1u(:,:) * e2u(:,:)
