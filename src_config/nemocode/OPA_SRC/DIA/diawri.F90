...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: diawri.F90 8473 2017-08-30 09:38:39Z timgraham $
...
# if defined key_iomput
   ...
   SUBROUTINE dia_wri( kt )
      ...
      !CALL wrk_alloc( jpi , jpj, nk_us , ZX3D ) !LOLO
      !
      ...
      !LOLO:
      IF( iom_use("e3u") ) CALL iom_put( "e3u" , fse3u_n(:,:,:) )
      IF( iom_use("e3v") ) CALL iom_put( "e3v" , fse3v_n(:,:,:) )
      IF( iom_use("e3w") ) CALL iom_put( "e3w" , fse3w_n(:,:,:) )
      !LOLO.
      !SLX:
      IF( iom_use("e1t") ) CALL iom_put( "e1t" , e1t(:,:) )
      IF( iom_use("e1u") ) CALL iom_put( "e1u" , e1u(:,:) )
      IF( iom_use("e1v") ) CALL iom_put( "e1v" , e1v(:,:) )
      !
      IF( iom_use("e2t") ) CALL iom_put( "e2t" , e2t(:,:) )
      IF( iom_use("e2u") ) CALL iom_put( "e2u" , e2u(:,:) )
      IF( iom_use("e2v") ) CALL iom_put( "e2v" , e2v(:,:) )
      !SLX.      
      ...
      !LOLO:
      !IF( iom_use('toce_us') ) THEN
      !   DO jk=1, nk_us
      !      jkt = jk_us(jk)
      !      ZX3D(:,:,jk) = tsn(:,:,jkt,jp_tem)
      !   END DO
      !   CALL iom_put( "toce_us", ZX3D(:,:,:) )    ! 3D temperature at user-specified vertical levels
      !   ZX3D(:,:,:) = 0.0_wp
      !END IF
      !LOLO.
      ...
      !LOLO:
      !IF( iom_use('soce_us') ) THEN
      !   DO jk=1, nk_us
      !      jkt = jk_us(jk)
      !      ZX3D(:,:,jk) = tsn(:,:,jkt,jp_sal)
      !   END DO
      !   CALL iom_put( "soce_us", ZX3D(:,:,:) )    ! 3D salinity at user-specified vertical levels
      !   ZX3D(:,:,:) = 0.0_wp
      !END IF
      !LOLO.
      ...
      !LOLO:
      !IF( iom_use('uoce_us') ) THEN
      !   DO jk=1, nk_us
      !      jkt = jk_us(jk)
      !      ZX3D(:,:,jk) = un(:,:,jkt)
      !   END DO
      !   CALL iom_put( "uoce_us", ZX3D(:,:,:) )
      !   ZX3D(:,:,:) = 0.0_wp
      !END IF
      !LOLO.
      ...
      !LOLO:
      !IF( iom_use('voce_us') ) THEN
      !   DO jk=1, nk_us
      !      jkt = jk_us(jk)
      !      ZX3D(:,:,jk) = vn(:,:,jkt)
      !   END DO
      !   CALL iom_put( "voce_us", ZX3D(:,:,:) )
      !   ZX3D(:,:,:) = 0.0_wp
      !END IF
      !LOLO.
      ...
      !LOLO:
      !IF( iom_use('woce_us') ) THEN
      !   DO jk=1, nk_us
      !      jkt = jk_us(jk)
      !      ZX3D(:,:,jk) = 0.5_wp*(wn(:,:,jkt)+wn(:,:,jkt+1)) !LOLO: vertical interpolation from W-point to T-point
      !   END DO
      !   CALL iom_put( "woce_us", ZX3D(:,:,:) )
      !   ZX3D(:,:,:) = 0.0_wp
      !END IF
      !LOLO.

      !loloCALL wrk_dealloc( jpi , jpj, nk_us , ZX3D ) !LOLO
