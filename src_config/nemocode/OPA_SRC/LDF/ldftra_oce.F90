MODULE ldftra_oce
   ...
   !LOLO:
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: zmu_lolo, zmv_lolo !LOLO

   REAL(wp), PUBLIC :: rthr_grad_sst, & ! [K/m] | threshold value from which |grad(SST)| is considered too extreme!!
      &                ramp_aht     , & ! multiplicative factor to ahtu,ahtv to apply where |grad(SST)| == rthr_grad_sst ! => can become larger where  |grad(SST)| > rthr_grad_sst !
      &                rmax_msk     , & !    => but never larger than rmax_msk
      &                rmin_msk         ! minimum value of the mask (background for the rest of the domain non problematic)
   
   INTEGER,  PUBLIC :: nt_often, nb_smooth_sst, nb_smooth_mask  ! !LOLO:  ! eNATL60 !!! 45 @40s => 30 minutes!
   !LOLO.
...
CONTAINS
   ...
   INTEGER FUNCTION ldftra_oce_alloc()
     ...
     ierr(:) = 0
      ...
      ALLOCATE( zmu_lolo(jpi,jpj,jpk) , zmv_lolo(jpi,jpj,jpk) , STAT=ierr(1) ) !LOLO
