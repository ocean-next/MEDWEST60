...
CONTAINS
   ...
   SUBROUTINE ldf_tra_init
      ...
      NAMELIST/namtra_ldf/ ln_traldf_lap  , ln_traldf_bilap,                  &
         ...
         &                 rn_aht_m       ,                                   &
         &                 rthr_grad_sst  , ramp_aht , rmax_msk , rmin_msk,   &  !LOLO
         &                 nt_often, nb_smooth_sst, nb_smooth_mask               !LOLO
      !!----------------------------------------------------------------------
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namtra_ldf in configuration namelist', lwp )
      ...
      IF(lwp) THEN                      ! control print
         ...
         !LOLO:
         IF ( ln_traldf_lap .AND. ln_traldf_hor ) THEN
            WRITE(numout,*) ' LOLO: grad(sst) threshold          rthr_grad_sst   = ', rthr_grad_sst
            WRITE(numout,*) ' LOLO: factor for grad(sst) mask         ramp_aht   = ', ramp_aht
            WRITE(numout,*) ' LOLO: max value for grad(sst) mask      rmax_msk   = ', rmax_msk
            WRITE(numout,*) ' LOLO: min value for grad(sst) mask      rmin_msk   = ', rmin_msk
            WRITE(numout,*) ' LOLO: freq. (jt) to update grad(sst) mask nt_often = ', nt_often
            WRITE(numout,*) ' LOLO: times smooth SST bef. grad(sst) nb_smooth_sst = ', nb_smooth_sst
            WRITE(numout,*) ' LOLO: times smooth grad(sst) mask   nb_smooth_mask = ', nb_smooth_mask
         END IF
         !LOLO.
