...
CONTAINS
   ...
   FUNCTION mynode( ldtxt, ldname, kumnam_ref , kumnam_cfg , kumond , kstop, localComm, kmember )
      !!----------------------------------------------------------------------
      !!                  ***  routine mynode  ***
      !!
      ...
      ! ensemble modification
      CHARACTER(len=256) :: cloutput
      !
      NAMELIST/nammpp/ cn_mpi_send, nn_buffer, jpni, jpnj, jpnij, ln_nnogather, &
         &             ln_ensemble, ln_ens_rst_in, nn_ens_size, nn_ens_start, ln_ens_diag
      !!----------------------------------------------------------------------
      !
      ii = 1
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nammpp in configuration namelist', lwp )
      ...
      WRITE(ldtxt(ii),*) '      ensemble simulation              ln_ensemble   = ', ln_ensemble   ;   ii = ii + 1
      WRITE(ldtxt(ii),*) '      ensemble restart                 ln_ens_rst_in = ', ln_ens_rst_in ;   ii = ii + 1
      WRITE(ldtxt(ii),*) '      ensemble size                    nn_ens_size   = ', nn_ens_size   ;   ii = ii + 1
      WRITE(ldtxt(ii),*) '      ensemble start                   nn_ens_start  = ', nn_ens_start  ;   ii = ii + 1
      WRITE(ldtxt(ii),*) '      ensemble diagnostic              ln_ens_diag   = ', ln_ens_diag   ;   ii = ii + 1
      ...
      IF( mynode == 0 ) THEN
         cloutput = TRIM(ldname)
         IF(ln_ensemble) THEN
            WRITE(cn_mem,'(I3.3)') kmember
            cloutput = cn_mem//TRIM(cloutput)
         ENDIF
         CALL ctl_opn( kumond, cloutput, 'UNKNOWN', 'FORMATTED', 'SEQUENTIAL', -1, 6, .FALSE. , 1 )
         WRITE(kumond, nammpp)      
      ENDIF
      !
   ...
   SUBROUTINE mpp_ens_ave_std( ksize, ptab, pave, pstd )
      !!----------------------------------------------------------------------
      !!                  ***  routine mpp_ens_ave_std ***
      !!
      ...
      !
      INTEGER , INTENT(in) :: ksize                             ! size of arrays
      ...
      CALL MPI_ALLREDUCE (ptab, pave, ksize, MPI_DOUBLE_PRECISION, &
                          MPI_SUM, localcomm, ierror)
      pave(1:ksize) = pave(1:ksize) / nn_ens_size

      ! compute ensemble standard deviation
      IF( PRESENT(pstd) ) THEN
         CALL wrk_alloc( ksize, ztab )

         ! compute anomalies with respect to the mean
         ztab(1:ksize) = ptab(1:ksize) - pave(1:ksize)
         ! compute squared anomalies
         ztab(1:ksize) = ztab(1:ksize) * ztab(1:ksize)

         ! compute ensemble mean squared anomalies
         CALL MPI_ALLREDUCE (ztab, pstd, ksize, MPI_DOUBLE_PRECISION, &
                             MPI_SUM, localcomm, ierror)
         pstd(1:ksize) = SQRT ( pstd(1:ksize) / ( nn_ens_size - 1 ) )

         CALL wrk_dealloc( ksize, ztab )
      ENDIF

   END SUBROUTINE mpp_ens_ave_std
   ...
   SUBROUTINE mppstop( ld_abort )
      ...
      !!
      !!----------------------------------------------------------------------
      LOGICAL, OPTIONAL :: ld_abort   ! ensemble modification
      INTEGER ::   info, ierr
      LOGICAL ::   labort
      !!----------------------------------------------------------------------
      !
...
CONTAINS
   ...
   SUBROUTINE ctl_opn( knum, cdfile, cdstat, cdform, cdacce, klengh, kout, ldwp, karea )
      ...
      IF( PRESENT( karea ) ) THEN
         ...
         IF( karea > 1 )   WRITE(clfile, "(a,'_',i5.5)") TRIM(clfile), karea-1
         !LOLO.
      ENDIF
#if defined key_agrif
