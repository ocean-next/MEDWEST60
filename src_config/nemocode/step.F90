...
# endif
      ...
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
       IF( ln_sto_hgr .OR. ln_sto_eos ) CALL sto_par( kstp )          ! Stochastic parameters
       IF( ln_sto_eos ) CALL sto_pts( tsn  )          ! Random T/S fluctuations
       IF( ln_sto_hgr ) CALL sto_hgr()
...
#  endif
      ...
      IF( ln_sto_hgr )   CALL sto_rst_write( kstp )   ! write restart file for stochastic parameters
